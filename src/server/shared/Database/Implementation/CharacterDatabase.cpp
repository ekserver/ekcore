...
bool CharacterDatabaseConnection::Open()
{
    ...
    PREPARE_STATEMENT(CHAR_LOAD_PLAYER_XPRATES, "SELECT UNIX_TIMESTAMP(start_time), UNIX_TIMESTAMP(end_time), kill_xp_rate, quest_xp_rate, explore_xp_rate, rest_xp_rate, premium FROM character_rates WHERE guid = ?", CONNECTION_ASYNC)
